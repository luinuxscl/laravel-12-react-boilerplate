...
it('filtra usuarios por rol y rango de fechas', function () {
    ...
    $u1 = User::factory()->create(['tenant_id' => $tenant->id, 'created_at' => now()->subDays(10)]); // sin rol
    $u2 = User::factory()->create(['tenant_id' => $tenant->id, 'created_at' => now()->subDays(5)]);  // Editor
    $u2->assignRole('Editor');
    $u3 = User::factory()->create(['tenant_id' => $tenant->id, 'created_at' => now()->subDays(1)]);  // Editor
    $u3->assignRole('Editor');

    // Filtro por rol
    $r1 = $this->actingAs($admin)->withHeaders(['X-Tenant' => $tenant->slug])->getJson('/admin/users?role=Editor');
    $r1->assertOk();
    ...
    $to = now()->toDateString();
    $r2 = $this->actingAs($admin)->withHeaders(['X-Tenant' => $tenant->slug])->getJson("/admin/users?created_from={$from}&created_to={$to}");
    $r2->assertOk();
